...
let N = Int(readLine()!)!
struct Member {
    let power, speed, tech, intelli, imagi: Int
    let _max: Int
}

var members = (0..<N).map { _ -> Member in
    let (p, s, t, i, im): (Int, Int, Int, Int, Int) = { let l = readLine()!.split(separator: " ").map(String.init); return (Int(l[0])!, Int(l[1])!, Int(l[2])!, Int(l[3])!, Int(l[4])!) }()
    return Member(power: p, speed: s, tech: t, intelli: i, imagi: im, _max: max(p, s, t, i, im))
}
members.sort { (l, r) -> Bool in
    return l._max > r._max
}

func score(members: [Member]) -> Int {
    var m_p = 0; var m_s = 0; var m_t = 0; var m_i = 0; var m_im = 0
    for m in members {
        m_p = max(m_p, m.power)
        m_s = max(m_s, m.speed)
        m_t = max(m_t, m.tech)
        m_i = max(m_i, m.intelli)
        m_im = max(m_im, m.imagi)
    }
    return min(m_p, m_s, m_t, m_i, m_im)
}
...
var ans = 0
for c in Combination(of: members, size: 3) {
    ans = max(ans, score(members: c))
}

print(ans)
//print(score(members: members))
