...
func main() {
    let (n, c) = cin.int2
    let abc: [(a: Int, b: Int, c: Int)] = cin.loopMap(count: n)
    
    // 合計
    var sum = 0
    
    // はじめとおわりの日
    let start = abc.map(\.a).min()!
    let end = abc.map(\.b).min()!
    
    for i in start..<end {
        // 全部のプランを見て、合計が c よりも大きければ c を加算する
        if
    }
